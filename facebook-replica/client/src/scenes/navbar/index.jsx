...
const Navbar = () => {
    ...
    // toggle mobile menu based on if on mobile
    const [isMobileMenuToggled, setIsMobileMenuToggled] = useState(false);
    const dispatch = useDispatch();     // dispatch actions from reducers
    const navigate = useNavigate();     // navigation between pages
    const user = useSelector((state) => state.user);        // extract data from redux store state
    const isNonMobileScreens = useMediaQuery("(min-width: 1000px)");        // media query to check within screen bounds
    
    const theme = useTheme();       // set theme
